...
import { PALETA_DE_CORES, POSICAO_CARREIRA, VENCIMENTO_BASICO, PONTOS_GEPI, VALOR_PONTO_GEPI, VALOR_DIARIO_VI, PERCENTUAL_PREVCOM_PADRAO, NIVEL_PADRAO, TETO_GEPI, TETO_SERVIDOR_PUBLICO, ANO_INGRESSO_PADRAO } from '../constants';
...
const ControleCenario: React.FC<propriedadesControleCenario> = ({ cenarios, adicionarCenario, atualizarCenario, removeCenario, anosProjecao, defineAnosProjecao }) => {
    ...
    const [parametrosCenarioAtuais, defineParametrosCenarioAtuais] = useState<parametrosDeCenario>({
    ...
    const gerenciarSalvamento = () => {
        if (estaEditando) {
            ...
            if (cenarioParaAtualizar) {
                atualizarCenario({ ...cenarioParaAtualizar, nome: nomeCenario, parametros: parametrosCenarioAtuais });
            }
        } else {
            adicionarCenario({
                id: uuidv4(),
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            <Card className="lg:col-span-1">
                ...
                <div className="space-y-4">
                    ...
                    <Select label="Nível na Carreira" id="posicaoCarreira" value={parametrosCenarioAtuais.posicaoCarreira} onChange={(e) => {
                    ...
                    <Input label="Ano de Ingresso" id="anoIngresso" type="number" min="0" value={parametrosCenarioAtuais.anoIngresso} onChange={(e) => defineParametrosCenarioAtuais(p => ({ ...p, anoIngresso: parseInt(e.target.value) || 0 }))} />
                    <Input label="Vencimento Básico (R$)" id="salarioBaseSobreposto" type="number" step="0.01" value={parametrosCenarioAtuais.salarioBaseSobreposto} onChange={(e) => defineParametrosCenarioAtuais(p => ({ ...p, salarioBaseSobreposto: parseFloat(e.target.value) || 0 }))} />
                    <Input label="Pontos GEPI" id="pontosGEPI" type="number" value={parametrosCenarioAtuais.pontosGEPI} onChange={(e) => defineParametrosCenarioAtuais(p => ({ ...p, pontosGEPI: parseFloat(e.target.value) || 0 }))} />
                    <Input label="Valor do Ponto GEPI (R$)" id="valorPontoGEPI" type="number" step="0.01" value={parametrosCenarioAtuais.valorPontoGEPI} onChange={(e) => defineParametrosCenarioAtuais(p => ({ ...p, valorPontoGEPI: parseFloat(e.target.value) || 0 }))} />
                    <Input label="Reajuste Geral Anual Médio (%)" id="RGAMedio" type="number" value={parametrosCenarioAtuais.RGAMedio} onChange={(e) => defineParametrosCenarioAtuais(p => ({ ...p, RGAMedio: parseFloat(e.target.value) || 0 }))} />
                    <Input label="Crescimento da GEPI Médio (%)" id="crescimentoGEPIMedio" type="number" value={parametrosCenarioAtuais.crescimentoGEPIMedio} onChange={(e) => defineParametrosCenarioAtuais(p => ({ ...p, crescimentoGEPIMedio: parseFloat(e.target.value) || 0 }))} />
                    <Input label="Teto da GEPI (× VB II-J)" id="tetoGEPI" type="number" value={parametrosCenarioAtuais.tetoGEPI} onChange={(e) => defineParametrosCenarioAtuais(p => ({ ...p, tetoGEPI: parseFloat(e.target.value) || 0 }))} />
                    <Input label="Teto do Servidor Público" id="tetoServidorPublico" type="number" value={parametrosCenarioAtuais.tetoServidorPublico} onChange={(e) => defineParametrosCenarioAtuais(p => ({ ...p, tetoServidorPublico: parseFloat(e.target.value) || 0 }))} />
                    <Input label="Dependentes de IR" id="dependentes" type="number" min="0" value={parametrosCenarioAtuais.dependentes} onChange={(e) => defineParametrosCenarioAtuais(p => ({ ...p, dependentes: parseInt(e.target.value) || 0 }))} />
                    <Input label="Dias Trabalhados (Ajuda de Custo)" id="diasTrabalhados" type="number" min="0" value={parametrosCenarioAtuais.diasTrabalhados} onChange={(e) => defineParametrosCenarioAtuais(p => ({ ...p, diasTrabalhados: parseInt(e.target.value) || 0 }))} />
                    <Input label="Valor Diário VI (R$)" id="valorVIDiaria" type="number" step="0.01" value={parametrosCenarioAtuais.valorVIDiaria} onChange={(e) => defineParametrosCenarioAtuais(p => ({ ...p, valorVIDiaria: parseFloat(e.target.value) || 0 }))} />
                    <div className="space-y-2 rounded-md border border-gray-200 dark:border-gray-700 p-3">
                        <div className="flex items-center space-x-2">
                            <input
                                type="checkbox"
                                id="prevcom"
                                checked={parametrosCenarioAtuais.prevcom}
                                onChange={(e) => defineParametrosCenarioAtuais(p => ({ ...p, prevcom: e.target.checked }))}
                        ...
                        {parametrosCenarioAtuais.prevcom && (
                            <Input
                                label="Participação PREVCOM (%)"
                                id="prevcomPercentage"
                                type="number"
                                step="0.01"
                                value={parametrosCenarioAtuais.percentualDeContribuicaoDaPrevcom}
                                onChange={(e) => defineParametrosCenarioAtuais(p => ({ ...p, percentualDeContribuicaoDaPrevcom: parseFloat(e.target.value) || 0 }))}
                            />
                        )}
                    </div>
                    <div className="flex items-center space-x-2">
                        <input
                            ...
                            checked={parametrosCenarioAtuais.filiadoAoSindicato}
                            onChange={(e) => defineParametrosCenarioAtuais(p => ({ ...p, filiadoAoSindicato: e.target.checked }))}
                            className="h-4 w-4 rounded border-gray-300 text-primary-600 focus:ring-primary-500"
                        />
